...
func main() {
	portEnvValue := os.Getenv("PORT")
	if portEnvValue == "" {
		portEnvValue = "3000"
	}
	port, err := parseNonNegativeIntegerString(portEnvValue)
	if err != nil {
	...
	var httpsServerFlag bool
	if httpsEnvValue == "1" {
		httpsServerFlag = true
	} else if httpsEnvValue == "0" {
		httpsServerFlag = false
	} else {
		log.Fatal("invalid HTTPS environment variable value")
	}
