...
func (server *serverStruct) createAuthSession(userId string) (authSessionStruct, []byte, error) {
	...
	err = sqlitex.Execute(
		...
		"INSERT INTO auth_session (id, user_id, secret_hash, created_at) VALUES (?, ?, ?, ?)",
		&sqlitex.ExecOptions{
			Args: []any{
				authSession.id,
				authSession.userId,
				authSession.secretHash,
				authSession.createdAt.Unix(),
			},
		},
	)
...
func (server *serverStruct) validateAuthSessionToken(SessionToken string) (authSessionStruct, error) {
	authSessionId, authSessionSecret, err := parseSessionToken(SessionToken)
	if err != nil {
...
func (server *serverStruct) validateRequestAuthSessionToken(r *http.Request) (authSessionStruct, string, error) {
	SessionTokenCookie, err := r.Cookie(authSessionTokenCookieName)
	if err != nil {
		return authSessionStruct{}, "", errInvalidAuthSessionToken
	}
	authSessionToken := SessionTokenCookie.Value
